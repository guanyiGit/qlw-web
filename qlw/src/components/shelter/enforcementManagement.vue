    ...
    <div class="w980 fr">
      <div class="ovh">
        <div class="fl cor6 h40 lh40">
          <router-link style="cursor:pointer;" class="db fl hov8 corffa472 mgr35 pc9 pdl36"
                       :to="{name:'hospice_illegalRegistration'}">违法登记
          </router-link>
        ...
        <form class="fr db w380 h40 borbfbfbf ras20 rel" action="#">
          <input type="text" class="db ww100 h38 lh38 pdl15 cor3c3c3c bort" placeholder="犬主姓名/犬主证件号/犬证号/免疫证号/设备编号" v-model="keyWord"
                 @keyup.enter="search">
        ...
        <div class="t-c">
          <table class="table5" width="100%" border="1">
            ...
            <tr v-for="(item,index) in lawEList" :key="index">
              ...
              <td>{{item.points}}</td>
              <td>{{item.isCollection==0?'是':item.isCollection==1?'否':'其他'}}</td>
              <td class="t-c">
                <router-link style="cursor:pointer;" class="dinb cor009cff hov8"
                             :to="{name:'enforcementManagementDetails',params:{info:JSON.stringify(item)}}">查看详情
                </router-link>
              </td>
            </tr>
            </tbody>
