  ...
  <div class="w980 fr">
    <div class="mbx">
      <ul class="ul11 clearfix">
        ...
        <li>{{info?info.infoTitile?info.infoTitile:'信息详情':'信息详情'}}</li>
      </ul>
    </div>
    <form action="#">
      <div class="ovh k3 mgt20">
        <div class="pdt23 pdb23 bgf1 t-c">
          <p class="fs20 coadsr0">{{info?info.infoTitile?info.infoTitile:'':''}}</p>
      ...
      methods:{
        ...
        notThrough(){
          this.$axios({
            ...
            method: 'post',
            params: {status: 2,informationId: this.$route.params.id,notifReceiverId:this.info.operatorId,infoTitile:this.info.infoTitile,failReason:this.why}
          }).then(res => {
        ...
        through(){
          this.$axios({
            url: '/biz/information/examine',
            method: 'post',
            params: {status: 1,informationId: this.$route.params.id,notifReceiverId:this.info.operatorId,infoTitile:this.info.infoTitile}
          }).then(res => {
