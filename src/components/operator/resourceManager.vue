    ...
    <el-dialog title="角色分配" :visible.sync="dialogFormVisible" width="450px">
      <el-form :model="edtBox">
        ...
        <el-form-item label="机构类型 " :label-width="formLabelWidth">
          <el-select v-model="edtBox.organization.orgType" placeholder="请选择要给用户分配的机构类型" class="text">
            <el-option :label="item.dictionaryDescribe" :value="item.dictionaryValue" v-for="(item,index) in orgTypes"
                       :key="index"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="机构名称 " :label-width="formLabelWidth"  v-if="orgTypeValue>=0">
          <el-select v-model="edtBox.organization.organizationId" placeholder="请选择要给用户分配的机构类型" class="text">
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      orgTypeValue(newVal) {
        this.edtBox.organization.organizationId = "";
        this.$axios({
    ...
    computed: {
      ...
      orgTypeValue(){
        return this.edtBox.organization.orgType;
      }
    ...
    methods: {
      ...
      addRoles() {
        ...
        //   }
        // })
        let temp = this.edtBox;
        this.$axios({
          url: '/biz/user/editRoleAndOrg',
          method: 'post',
          headers: {
            'content-type': 'application/json'
          },
