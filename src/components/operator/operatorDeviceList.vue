        ...
        <table class="table11" width="100%">
          ...
          <tr v-for="(item,index) in list" :key="index" v-if="list && list.length>0">
            ...
            <td>
              ...
              &nbsp; &nbsp; &nbsp;<router-link :to="{name:'operator_device_info',params:{'devId':item.deviceId}}"
  ...
  export default {
    ...
    watch: {
      checkTab(newVal, oldVal) {
        if (newVal != oldVal) {
          // newVal = oldVal
          params.deviceStatus = oldVal;//切换tab
          initData(this)
        }
      },
