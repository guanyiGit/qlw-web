  ...
  <div class="w1200 c-b ovh mgt40">
    ...
    <div class="fr w414">
      ...
      <div class="mgt30">
        ...
        <div class="pdt10 scroll-wrap">
          <ul class="ul04 scroll-content" :style="{ top }" style="margin-top: -13px">
            <!--网点列表-->
              ...
              <!--<a class="db ovh" @click="$message('未开通')">--><!--:to="{name:'checkBranches'}"-->
                ...
                <div class="db ovh">
                  <router-link :to="{name:'HospitalDetails',params:{id:item.organizationId}}">
                    ...
                    <div class="fl w122"><img :src="item.iconUrl" alt="" style="width: 123px;height: 123px"></div>
                  </router-link>
                  <div class="mgl143 rel pdt12">
                    ...
                    <router-link :to="{name:'serviceGuide',params:{type:1,orgId:item.organizationId,orgName:item.organizationName}}"><span class="db abs z100 w65 h28 lh28 t-c corw sp1">预约</span></router-link>
                  </div>
                </div>
...
<script>
  let params = {
    ...
    data2:{provinceId : null,ownedCityId : null,districtId : null,orgtype : 2,status : 1,pageNum:1,pageSize:2000}
    //服务网点参数
  ...
  export default {
    ...
    beforeCreate() {
      ...
      setInterval(_ => {
        if (this.activeIndex < this.hospitalList.length-1) {
          this.activeIndex += 1;
        } else {
          this.activeIndex = 0;
        }
    ...
    computed: {
      top() {
        return - this.activeIndex * 143 + 'px';
      }
    },
  }
  ...
  function initHospitalList(_this) {
    _this.$axios({
      ...
      if (res.data.status == 200 && res.data.data != null) {
        res.data.data.list.forEach((item,i) => {
          ...
          if(item.pictureUrl.length > 0){
            iconUrl = item.pictureUrl[0].thumbnailUrl;
          }else {
            iconUrl = "/static/images/17-1.png";
          }
      ...
      console.log(_this.hospitalList[0].pictureUrl[0].thumbnailUrl)
    }).catch((err) => {
      console.log(err);
    })
  }
