  ...
  <div class="w980 fr">
    <div class="ovh">
      <div class="fl cor6 h40 lh40">
        ...
        <router-link class="db fl hov8 corffa472 mgr35 pc9 pdl36" :to="{name:addpath}">新增</router-link>
          ...
          <tr v-for="(item,index) in userList" :key="index">
            ...
            <td class="t-c">
              ...
              <router-link class="dinb cor009cff hov8 mgr14"
                           :to="{name:checkpath,params:{blackId:item.blackId}}">查看详情
              </router-link>
  ...
  export default {
    ...
    created() {
      ...
      if (roleIds && roleIds.length == 1) {
        if (roleIds[0]==1) {//犬只办
          this.checkpath='dogManager_blackinfo';
          this.addpath='dogManager_blackadd';
        }else if (roleIds[0]==2) {//收容所
          this.checkpath='hospice_blackinfo';
          this.addpath='hospice_blackadd';
        }else if (roleIds[0]==7) {//电信平台用户
          this.checkpath='telecom_blackinfo';
          this.addpath='telecom_blackadd';
        }
      }
    ...
    computed: {
      pageData() {
        return {
          ...
          totalCount: this.totalPage ? this.totalPage : 0,//总条数
    ...
    _this.$axios({
      ...
      if (res.data.status == 200) {
        if (res.data.data == null) {
          ...
          _this.totalPage = res.data.data.totalRecord
        }
      }
    }).catch((res) => {
      console.log(res)
