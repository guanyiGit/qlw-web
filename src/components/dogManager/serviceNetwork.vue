  ...
  <div class="w980 fr" >
    ...
    <!--<div class="div12 dn mgt20">
      <div class="ovh">
        <div class="fl cor6 h40 lh40">
          &lt;!&ndash;<router-link :to="{name:'operator_msgAdd'}" class="db fl hov8 corffa472 mgr35 pc9 pdl36">新增</router-link>&ndash;&gt;
        </div>
      ...
      <div class="ovh k3 mgt20">
        <table class="table11" width="100%">
          ...
          <tr v-for="(item,index) in  stayList" :key="index">
            ...
            <td>
              <router-link style="cursor:pointer;" class="dinb hov8 cor009cff" :to="{name:'operator_networkAudit',params:{'organizationId':item.organizationId}}">审核</router-link>
            </td>
          </tr>
    ...
    <div class="div12 db mgt20">
      <div class="ovh">
        ...
        <router-link style="cursor:pointer;" class="fl db fs16 cor3c3c3c w200 h50 lh50 t-c active" :to="{name:'dogManager_serviceNetworkMap',params:{id:62}}">查看地图</router-link>

        <!--<a href="javascript:void(0);" class="fl db fs16 cor3c3c3c w200 h50 lh50 t-c active">查看地图</a>-->
      </div>
      <div class="ovh k3 mgt20">
        <table class="table11" width="100%">
          <tbody>
          ...
          <tr v-for="(item,index) in  stayList1" :key="index" v-if="stayList1 && stayList1.length>0">
            ...
            <!--</td>-->
            <td>
              <!--<router-link style="cursor:pointer;" class="dinb hov8 cor009cff" :to="{name:'dogManager_serviceNetworkDetails',params:{'organizationId':item.organizationId}}">查看</router-link>-->
      ...
      toCheck(organizationId){
        this.$axios({
          ...
          if (res.data.status == 200) {
            this.$router.push({name:'dogManager_serviceNetworkDetails',params:{'organizationId':organizationId,province:res.data.data.province,city:res.data.data.ownedCity,district:res.data.data.district}})
          }
        })
      }
    },
  ...
  function initData1(_this) {
    _this.$axios({
      ...
      if(res.data.status == 200){
        ...
        console.log("服务网点："+JSON.stringify(_this.stayList1))
      }
    })
  }
</script>
