    ...
    <div class="mgt10 k3">
      ...
      <div class="t-c">
        <table class="table11 t-c" width="100%">
          <tbody>
          <tr>
            <td style="width: 13%;">犬只名称</td>
            <td style="width: 10%;">犬只品种</td>
            <td style="width: 6%;">性别</td>
            <td style="width: 6%;">犬龄</td>
            <td style="width: 10%;">毛色</td>
            <td style="width: 10%;">犬主姓名</td>
            <td style="width: 10%;">联系电话</td>
            <!--<td>下次喂药时间</td>-->
            <td style="width: 13%;">免疫证号</td>
            <td style="width: 10%;">创建时间</td>
            <td style="width: 10%;">操作</td>
          </tr>
          <tr v-for="(item,index) in therapyList" :key="index">
            ...
            <td v-else>···</td>
            <td v-if="item.creation_time">{{item.creation_time|formatDate(dataFormatPattern)}}</td>
            <td v-else>···</td>
            <td>
              <router-link class="dinb cor009cff hov8" :to="{name:'hospital_txyinfo',params:{'dogId':item.dog_id}}">病历记录</router-link>
            </td>
          </tr>
...
<script>
  ...
  function initData(_this) {
    ...
    _this.$axios({
      ...
      console.log("病历"+JSON.stringify(res.data))
      if (res.data.code==200) {
        _this.therapyList = res.data.list;
  ...
  function getDogList(_this){
    ...
    _this.$axios({
      ...
      if(ref.data.code==200){
        ...
        _this.therapyList=ref.data.list;
