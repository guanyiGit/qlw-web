<template>
  ...
  <div class="w980 fr">
    ...
    <div style="margin-left: 45%;font-size: 20px">
      <div style="width: 12%;background-color: #FFA54F;border-radius: 3px;margin-top: 2%">
        <a href="javascript:" style="margin-left: 15%" @click="printimm">打 印</a>
      </div>
    </div>
    <!--弹窗   说明。.db--显示，.dn--隐藏-->
    ...
    <div id="printimm" style="display: none">
      <div style="width: 800px;background-color: #F0F0F0;">
        <div style="width: 100%;text-align: center;">
          <p style="padding-top: 20px;padding-bottom: 20px;font-size: 18px">犬免疫登记卡</p>
        </div>
        <div style="width: 100%;text-align: center">
          <div style="width: 80%;margin-left: 10%;border: #3c3c3c 1px solid;background-color: #fffefe">
            <table class="immtable10 t-c" width="100%" border="1">
              <tbody>
              <tr>
                <td style="width: 23%">犬主姓名</td>
                <td class="cor9" style="width: 23%"v-if="Datails&&Datails.dog_owner_name">{{Datails.dog_owner_name}}</td>
                <td class="cor9" style="width: 23%"v-else>···</td>
                <td style="width: 23%">联系电话</td>
                <td class="cor9" style="width: 23%"v-if="Datails&&Datails.dog_owner_phone">{{Datails.dog_owner_phone}}</td>
                <td class="cor9" style="width: 23%"v-else>···</td>
              </tr>
              <tr>
                <td>详细地址</td>
            ...
            <table class="immtable10 t-c" width="100%" border="1">
              <tbody>
              <tr>
                <td colspan="11" style="border-top: #3c3c3c 1px solid;border-bottom: #3c3c3c 1px solid">健康状态检查</td>
              </tr>
              <tr>
                <td style="width: 9%">检查日期</td>
                <td style="width: 9%">皮温</td>
                <td style="width: 9%">肛温</td>
                <td style="width: 9%">脉搏</td>
              ...
              <tr v-for="(item,index) in healthConditionList">
                <td>{{item.inspect_time|formatDate(dataFormatPattern)}}</td>
                <td>{{item.skin_temperature}}</td>
                <td>{{item.anal_temperature}}</td>
                <td>{{item.pulse}}</td>
                <td>{{item.breathe}}</td>
                <td>{{item.visible_mucous_membrane}}</td>
                <td>{{item.skin}}</td>
                <td>{{item.weight}}</td>
                <td>{{item.inspect_org}}</td>
                <td>{{item.operator}}</td>
                <td>{{item.remark}}</td>
              </tr>
              </tbody>
            ...
            <table class="immtable10 t-c" width="100%" border="1">
              <tbody>
              <tr>
                <td colspan="7" style="border-top: #3c3c3c 1px solid;border-bottom: #3c3c3c 1px solid">免疫记录</td>
              </tr>
              <tr>
        ...
        <div style="padding-bottom: 20px;">
          <div style="margin-left: 11%">
            <span>免疫单位：{{confs&&confs.userInfo&&confs.userInfo.tOrganization&&confs.userInfo.tOrganization.organizationName
              ?confs.userInfo.tOrganization.organizationName:'...'}}</span><span style="margin-left: 50%">初始记录日期：</span>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    ,data(){
      ...
      ,tableCancel(){//点击取消（关闭新增）事件
        ...
        console.log("免疫登陆信息"+JSON.stringify(this.Datails))
        //return
  ...
  function initData(_this) {
    console.log(_this.$route.params.VIId)
    ...
    _this.$axios({
      url:'/biz/dogInfo/getHealthCondition'
      ,params:{dogId:_this.$route.params.dogId}
    }).then((ref)=>{
      if(ref.data.code==200){
        _this.healthConditionList=ref.data.list;
        console.log("健康检查集合："+JSON.stringify(_this.healthConditionList))
      }
    })
  }
