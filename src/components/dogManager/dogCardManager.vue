          ...
          <div class="t-c">
            <table class="table5" width="100%" border="1">
              ...
              <tr v-for="(item,index) in infos1" :key="index">
                ...
                <td class="t-c">
                  <router-link v-if="item.status!=4" class="db cor009cff hov8" :to="{name:path,params:{'dogId':item.dogId,'name':'证件办理'}}">查看详情</router-link>
  ...
  import  conf from '../../common/config'
    export default {
      ...
      created() {
        ...
        if (roleIds && roleIds.length == 1) {
          if (roleIds[0] == 1) {
            ...
            this.path = 'telecom_info';
            this.path1 = 'telecom_dogadd';
            this.path2 = 'telecom_dogCardaudit';
          }
        }
        initData(this)
        initData1(this)
      ...
      methods:{
        ...
        renewConfirm(){//续费确认
          let alertMdg = "续期失败";//提示窗信息
          if(this.alert.selectdDate){
            if (this.alert.selectdDate<new Date()){
              this.$alert("续期期限不能小于当天")
              return
            }
            this.$axios({//续费请求
