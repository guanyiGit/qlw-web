  ...
  <div class="w980 fr pdb40">
    <div class="ovh mgt20">
      <div class="fl cor6 h40 lh40">
        <router-link :to="{name:'hospital_reservationAdd'}" class="db fl hov8 corffa472 mgr35 pc9 pdl36">预约登记
        </router-link>
      </div>
      <form class="fr db w380 h40 borbfbfbf ras20 rel" action="#">
        <input type="text" class="db ww100 h38 lh38 pdl15 cor3c3c3c bort" placeholder="预约人电话/姓名" v-model="keyword">
      ...
      <div class="div12 db">
        <table class="table11">
          ...
          <tr v-for="(item,index) in list" :key="index">
            <td v-if="item.scheduleTime">{{item.scheduleTime | formatDate(pattern)}}</td>
            <td v-else>--</td>
  ...
  export default {
    ...
    methods: {
      ...
      changedStatus(index) {
        ...
        // let appointmentId = 1
        // let status = 0
        let str = status == 1 ? "已赴约" : '未赴约';
        this.$confirm("确认变更预约状态？", "状态变更", {
          confirmButtonText: '改为"' + str + '"',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(_ => {
          this.$axios({
