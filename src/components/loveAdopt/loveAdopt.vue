<template>
  <div>
    ...
    <div class="w1200 c-b mgt40">
      ...
      <ul class="ul08 mgt10">
        <li v-for="item in dogs" class="fl w230">
          <div style="cursor:pointer;" @click="toApply(item.collectionId,item.adoptionPublicityId)">
            <img class="ww100" style="height: 200px" v-bind:src="item.thumbnailUrl" alt="">
  ...
  export default {
    ...
    data() {
      return {
        ...
        dogs: [],
        totol: null,
        data:{page: 1, rows: 12, string: null},
        pageData: {
          curPage:1,//当前页吗
          totalCount:1,//总条数
          pageSize:1//页面大小
    ...
    methods: {
      ...
      toApply: function (id,dogPublicityId) {
        this.$router.push({name: 'adoptt', params: {collectionId: id,dogPublicityId:dogPublicityId}})
      },
      chekdpage(pageNum) {
  ...
  function picturePage(string) {
    ...
    _this.$axios({
      ...
      if (res.status == 200) {
        ...
        _this.pageData.totalCount = res.data.records
        _this.pageData.curPage= _this.data.page//当前页吗
        _this.pageData.pageSize= _this.data.rows//页面大小
      }
    })
  }
