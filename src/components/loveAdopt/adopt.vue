            ...
            <div class="ovh">
              ...
              <div class="mgt10 w550 fr">
                ...
                <div class="mgl80">
                  ...
                  <input id="dj" type="text" class="fr ww20 h40 lh40 borf1 ras4 t-c bgfbad82 corw" value="点击获取" v-on:click="getCode">
                  <span class="fr ww15 h40 lh40 borf1 ras4 t-c bgfbad82 corw" id="cod" style="display: none;">{{seconds}}s</span>
                </div>
              </div>
            </div>
...
<script>
  ...
  function submitApply() {
    if(thiss.code==null){
      thiss.$message("请获取验证码！");
      return;
    }
    if(thiss.code!=thiss.codee){
      thiss.$message("验证码错误！");
      return;
    }
    if(thiss.person.applyerName==null || thiss.person.applyerName==''){
      thiss.$message("领养人姓名不能为空");
      return;
    }
    if(thiss.person.applyerPhone==null || thiss.person.applyerPhone==''){
      thiss.$message("联系电话不能为空");
      return;
    }
    if(!thiss.person.applyerPhone || !/^1[34578]\d{9}$/.test(thiss.person.applyerPhone)){
      thiss.$message("电话号码格式有误!");
      return;
    }
    thiss.$axios({
      method: 'post',
  ...
  function getDogInfo(_this) {
      if(_this.$route.params.collectionId==null||_this.$route.params.collectionId==''){
        _this.$alert("信息失效，返回上一页")
        _this.$router.push({name:'loveAdopt'})
      }
    _this.$axios({
      method: 'get',
