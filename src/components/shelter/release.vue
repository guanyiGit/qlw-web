    ...
    <div class="w980 fr">
      <div class="mbx">
        <ul class="ul11 clearfix">
          <li>
            <a style="cursor:pointer;" @click="fromReset">信息列表  > </a>
          </li>
          <li>信息发布</li>
        </ul>
      </div>
              ...
              <div class="mgt10 ww100">
                ...
                <div class="mgl60">
                  <div style="width: 100%;">
                    ...
                    <div id="editor" style="width:100%;height:400px;position:relative;z-index: 0"></div>
                    </div>
                    </div>
                    </div>
                    </div>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      parms.information.operatorId=config.userInfo.userId
      console.log()
    },
    methods: {
          save:function() {
            console.log(parms.information)
            getpic();
            //查图片id
            this.$axios({
              method:'post',
              ...
              if (res.data.code == "000000") {
                parms.imgsrcs=[]
                if (res.data.result!=null && JSON.stringify(res.data.result)!="[]") {
...
//录入数据
  function inputData(_this) {
    ...
    console.log(parms.information)
    if(parms.information.infoTitile==null||parms.information.infoTitile==''){
      _this.$message("请输入标题")
      return
    }
    if(parms.information.infoContent==null||parms.information.infoContent=='<p><br></p>'){
      _this.$message("请输入内容")
      return
    }
    _this.$axios({
      ...
      data:JSON.stringify({information:JSON.stringify(parms.information),imageIds:JSON.stringify(parms.list)})
    }).then((res) => {
      console.log(res)
      if (res.data.status == 200) {
        _this.$alert("发布信息成功！").then(_ => {
          _this.$router.back(-1);
        }).catch(_ => {
        });
      }
