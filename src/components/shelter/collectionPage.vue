...
//全市犬只趋势
      ...
      .then((res) => {
        if (res.data.meta.success) {
            ...
            if(data.length>0){
              ...
              var count=[];
              for(var i=0;i<data.length;i++){
                date0.push(data[i].date);
                count.push(data[i].dogCount)
              }
            }
          _this.staticDogAll.staticDogAllCount=count;
          _this.staticDogAll.staticDogAllDate=date0;
        }
      })
  }
  ...
  function findDogIllegalCountsTendency(_this) {
    _this.$axios({url: '/biz/statistic/findDogIllegalCountsTendency', params: {"count": _this.month2 * 12}})//TODO 月数
      .then((res) => {
        if (res.data.meta.success) {
            let data=res.data.data.totalIllegalCountsTendency;
